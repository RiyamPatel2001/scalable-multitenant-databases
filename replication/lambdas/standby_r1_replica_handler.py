...
read_only_s3 = boto3.client('s3', region_name='us-east-1')
primary_s3 = boto3.client('s3', region_name='us-east-2')


def lambda_handler(event, context):
    """
    Lambda handler for replicating database snapshots to the read-only bucket.
    ...
    for record in event['Records']:
        try:
            ...
            read_only_bucket = sns_message.get('read_only_bucket')
            ...
            print(f'Destination: {read_only_bucket}/{db_path}')
            
            # Validate required fields
            if not all([snapshot_bucket, snapshot_s3_key, read_only_bucket, db_path]):
                print('ERROR: Missing required fields in message')
                raise ValueError('Missing required fields: snapshot_bucket, snapshot_s3_key, read_only_bucket, or db_path')
            
            # Download snapshot file from primary bucket
            ...
            try:
                ...
                primary_s3.download_file(snapshot_bucket, snapshot_s3_key, tmp_snapshot_path)
                
            except Exception as e:
                print(f'ERROR: Failed to download snapshot from S3: {str(e)}')
                raise
            
            # Upload snapshot to read-only bucket
            try:
                print(f'Step 3: Uploading to read-only bucket...')
                
                read_only_s3.upload_file(tmp_snapshot_path, read_only_bucket, db_path)
                
                print(f'Snapshot uploaded successfully to read-only bucket')
                
            except Exception as e:
                print(f'ERROR: Failed to upload snapshot to read-only bucket: {str(e)}')
                raise
            ...
            print(f'  Replicated to: {read_only_bucket}/{db_path}')
    ...
    return {
        'statusCode': 200,
        'body': json.dumps({
            'message': 'Replication processing completed',
